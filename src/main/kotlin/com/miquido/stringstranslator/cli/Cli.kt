...
private const val HELP_PROLOGUE =
    "Use this program to convert Android/iOS strings files to spreadsheet or to " +
...
fun parseConfigFromCliArgs(args: Array<String>): Config {
    return ArgParser(args, helpFormatter = DefaultHelpFormatter(prologue = HELP_PROLOGUE))
        ...
        .run {
            ...
            GlobalContext.loadKoinModules(module = loggerModule)
...
class Cli(parser: ArgParser) : KoinComponent {
    ...
    fun parseConfigFile(configPath: String): Config {
        ...
        when (val validationResult = configProperties.validate(mode)) {
    ...
    companion object {
        ...
        private const val MODE_HELP =
            "2 match mode - use toSpreadsheet to convert platform specific strings to" +
